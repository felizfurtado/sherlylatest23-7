...
def update_allblogs(blog_data, blog_filename):
    ...
    first_image = next((item['image'] for item in blog_data['content'] if 'image' in item), "/static/images/default.png")
...
def index():
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.docx'):
            ...
            # Create the complete blog data structure
            blog_data = {
                'id': get_next_id(),
                'title': title,
                'category': category,
            ...
            # Update allblogs.json
            update_allblogs(blog_data, json_filename)
